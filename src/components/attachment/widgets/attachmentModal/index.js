...
const AttachmentModal = (props) => {
    ...
    const { data: attachmentsData, refetch } = useQuery({queryKey: [attachment], queryFn: () => request({ url: attachment, params: {  search, sort: sorting, paginate: paginate, page ,...paramsProps } },router), enabled: false, refetchOnWindowFocus: false, select: (data) => data?.data });
                                    ...
                                    <Form className="theme-form theme-form-2 mega-form">
                                        <div>
                                            <div className="dflex-wgap justify-content-center ms-auto save-back-button">
                                                <h2>{t("drop_files_here")} <span>{t("or")}</span>
